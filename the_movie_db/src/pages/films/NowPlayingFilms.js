...
function NowPlayingFilms() {
  ...
  const { data: films, loading: loadingFilms, error: errorFilms } = useFetch(`https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.REACT_APP_API_KEY_TMDB}&language=fr-FR&page=1`);
  ...
  return (
    <main>
      {(loadingFilms || loadingGenres)
        ...
        : <MainContainer>
          <TitlePage>Films du moment</TitlePage>
          <Container>
            <FiltersContainer>
              <Filters>
